...
新流程改为“人物/物体轨迹 S 版本 → 人工批准 → Agent 摄像机规划 → 人工批准 → Blender 三机位 Proxy”。计划和渲染作业都绑定获批 S 版本的 SHA-256；修改 S 版本会取消当前计划与结果指针，但保留历史目录。

本次只运行一条短的真实 Blender 集成检查，没有调用 DeepSeek 或视频生成 API。A/B/C 三路 MP4 均实际解码为 3 帧、3 FPS、160×90，三路哈希互不相同；每路真实产生 Depth 和 CryptoObject00/01/02 结构帧，自动几何检查通过。该结果仅证明软件链路和绑定有效，不代表用户对构图的人工批准。
