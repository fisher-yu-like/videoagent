...
本原则适用于阶段 0 至项目结束的所有模块、实验、训练和评测：

- 不得用 mock/fake API、虚构任务 ID、编造响应 JSON、合成成功日志或伪造输出视频作为阶段通过证据；
- 本地单元测试、静态扫描、dry-run 和合成小样只能标注为“代码检查”或“管线检查”，不能证明真实模型、API、服务器、数据或研究假设有效；
- API 能力必须由用户批准的真实请求、真实任务状态、真实响应和真实生成文件验证；
- 服务器与训练能力必须由真实硬件日志、真实数据批次、真实 checkpoint、真实验证集指标和实际费用/时长验证；
- 使用合成 Blender 数据时必须明确标注为合成数据，并额外使用真实视频或真实 API 输出做域外验证；
- 回归样本可以从真实响应脱敏派生，但必须保存来源时间、模型、run 路径和原始文件哈希，不得手写一个看似真实的响应；
- 如果真实数据、权限、费用或接口文档不足以做出判断，结果必须标为“未验证”，立即报告用户并等待决策，不得自行补造证据；
- 阶段报告必须把“本地代码检查”和“真实外部验证”分栏呈现，避免把两者混为一谈。

### 0.6 2026-07-29 执行授权更新

用户授权在既定设计范围内自主推进本地阶段，不再为普通文件修改和本地检查逐次等待。以下约束继续有效：

- 每个阶段仍需报告真实结果、失败和未验证项；
- 每个生成后端每次验证最多提交一个付费 smoke task，禁止自动重试付费提交，成功或失败后立即停止批量调用；
- 如果密钥、真实数据或接口权限不可用，明确标记“未验证”并继续不依赖该外部条件的模块，不得恢复已泄露密钥或伪造外部结果；
- 远程服务器/GPU 的租用与付费仍需在使用前报告配置、时长和预算；
- 任何超出既定设计、明显增加费用、上传真实人物素材或改变外部系统状态的行为仍需单独确认。
