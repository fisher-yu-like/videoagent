...
打开 `http://127.0.0.1:8769`。先播放完整 Proxy，再选择“锁定起点”：例如选择 K2 会锁定 K0–K2，并为 K3/K4 自动初始化当前 Proxy 的完整镜头参数。你只需画人物点并按需修改镜头；要改 K2，应选择 K1。每帧不再要求人工动作 Prompt，轨迹完成后可显式预览由本地确定性编译器生成的只读 Prompt。页面会区分继承镜头与人工修改，并在保存时列出具体缺失项。
